...
def register_user(message, bot, user, cont):
    ...
    if message.from_user.id == user.id:
        ...
        if cont == 9:
            ...
            if message.content_type == "text":
                check_swear_words(message, bot)
            bot.register_next_step_handler(message, register_user, bot, user, cont + 1)
...
def show_warnings(message, bot, user, cont):
    ...
    if message.from_user.id == user.id:
        ...
        if cont == 9:
            ...
            if message.content_type == "text":
                check_swear_words(message, bot)
            bot.register_next_step_handler(message, show_warnings, bot, user, cont + 1)
        ...
        if (bot.get_chat_member(message.chat.id, message.from_user.id).status not in ["creator", "administrator"]) or (message.from_user.id != MY_CHAT_ID):
            if message.chat.type == "private":
                ...
                if len(results) == 0:
                    ...
                    if results[0][2] == 2:
                        #Baneado del grupo
                        banEnd = datetime.now() + timedelta(days = 1) 
                        ...
                        cursor.execute(query)
                        conn.commit()
                        bot.send_message(message.chat.id, "El usuario " + user + " ha sido baneado del grupo hasta el día " + banEnd.strftime("%d/%m/%y") + " a las " + banEnd.strftime("%H:%M") + ".")
...
def unban_user(message, bot, user, cont):
    ...
    if message.from_user.id == user.id:
        ...
        if cont == 9:
            ...
            if message.content_type == "text":
                check_swear_words(message, bot)
            bot.register_next_step_handler(message, unban_user, bot, user, cont + 1)
